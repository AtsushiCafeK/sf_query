...
`config.yaml` の書き間違いは、エラー画面（500）や無反応にはならず、**原因が画面に出る**。

| 設定ミス | 表示される内容 |
|---|---|
| YAMLのインデント崩れ | 書式エラーと**行番号** |
| 項目名・演算子・型の誤り | どのフィルタの何が不正か |
| キー名の綴り誤り（`filters` → `filter` 等） | 見慣れない項目として警告（黙って機能が消えるのを防ぐ） |
| 必須設定の欠落 | 不足している設定名 |

修正して保存したら、画面を再読み込みするだけで反映される。
...
| モード | 内容 | 状況 |
|---|---|---|
| `oauth`（配布時の既定） | **OAuth 2.0 認可コードフロー + PKCE**。利用者ごとにブラウザでログイン | ✅ 本番用（接続アプリの作成が必要） |
| `cli` | **sf CLI パススルー**（`sf api request rest`）。sf が資格情報を保管・更新し、アプリはトークンを一切扱わない。接続アプリ不要 | ✅ 開発用（Salesforce CLI が必要） |
...
5. ✅ ローカルWeb UI
6. ✅ OAuth 2.0 認可コードフロー+PKCE（画面内の接続ボタン）… 疎通確認済み
7. ✅ TeamSpirit本番接続・検証 … config のAPI名差し替えのみで動作
8. ⬜ 今後: 進捗表示・大量件数対応（Bulk API）・配布（exe化等）
